...
namespace TCLAP
{
      ...
      public:
        ...
        ArgContainer& add(Arg* arg)
        {
            ...
            for (iterator it = begin(); it != end(); it++)
            {
                if (*arg == **it)
                {
                    throw SpecificationException("Argument with same flag/name already exists!", arg->longID());
                }
            }
    ...
    inline void CmdLine::_constructor()
    {
        ...
        SwitchArg* ignore =
            new SwitchArg(Arg::flagStartString(), Arg::ignoreNameString(), "Ignores the rest of the labeled arguments following this flag.", false, v);
    ...
    inline void CmdLine::parse(std::vector<std::string>& args)
    {
        ...
        try
        {
            ...
            */
            ...
            for (std::list<ArgGroup*>::iterator it = _argGroups.begin(); it != _argGroups.end(); ++it)
            {
    ...
    inline void CmdLine::missingArgsException(const std::list<ArgGroup*>& missing)
    {
        ...
        for (std::list<ArgGroup*>::const_iterator it = missing.begin(); it != missing.end(); it++)
        {
            missingArgList += (*it)->getName();
            missingArgList += ", ";
            count++;
    ...
    inline void CmdLine::ignoreUnmatched(const bool ignore) { _ignoreUnmatched = ignore; }
