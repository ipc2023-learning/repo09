...
namespace TCLAP
{
    ...
    namespace internal
    {
        ...
        {
            return !(arg.getName() == Arg::ignoreNameString() || arg.isValueRequired() || arg.getFlag() == "") && arg.visibleInHelp();
        }
        ...
        {
            return (arg.getName() != Arg::ignoreNameString() && arg.isValueRequired() && arg.hasLabel() && arg.visibleInHelp());
        }
    ...
    inline void StdOutput::_shortUsage(CmdLineInterface& _cmd, std::ostream& os) const
    {
        ...
        // example.
        for (std::list<ArgGroup*>::iterator it = exclusiveGroups.begin(); it != exclusiveGroups.end();)
        {
        ...
        // stuck together).
        for (std::list<ArgGroup*>::iterator sit = nonExclusiveGroups.begin(); sit != nonExclusiveGroups.end(); ++sit)
        {
        ...
        std::sort(longSwitches.begin(), longSwitches.end(), internal::CompareShortID);
        for (std::vector<Arg*>::const_iterator it = longSwitches.begin(); it != longSwitches.end(); ++it)
        {
            outp << " [" << (**it).shortID() << ']';
        }

        // Now do all exclusive groups
        for (std::list<ArgGroup*>::iterator sit = exclusiveGroups.begin(); sit != exclusiveGroups.end(); ++sit)
        {
            ArgGroup& argGroup = **sit;
            outp << (argGroup.isRequired() ? " {" : " [");

            std::vector<Arg*> args;
            for (ArgGroup::iterator it = argGroup.begin(); it != argGroup.end(); ++it)
            {
                if ((**it).visibleInHelp())
                {
                    args.push_back(*it);
            ...
            std::string sep = "";
            for (std::vector<Arg*>::const_iterator it = args.begin(); it != args.end(); ++it)
            {
                outp << sep << (**it).shortID();
                sep = "|";
            }
        ...
        for (std::vector<std::pair<const Arg*, bool>>::const_iterator it = options.begin(); it != options.end(); ++it)
        {
    ...
    namespace
    {
        inline void fmtPrintLine(std::ostream& os, const std::string& s, int maxWidth, int indentSpaces, int secondLineOffset)
        {
            ...
            for (;;)
            {
                ...
                int tooFar = to;
                while (tooFar - from <= maxChars && static_cast<std::size_t>(tooFar) != std::string::npos)
                {
                    to = tooFar;
