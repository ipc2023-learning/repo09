...
#include "policy_iterative_tabular_evaluation.hpp"
...
{
    PolicyIterativeTabularEvaluation::PolicyIterativeTabularEvaluation(uint32_t batch_size,
                                                                       uint32_t chunk_size,
                                                                       uint32_t max_epochs,
    ...
    void PolicyIterativeTabularEvaluation::initialize_tables_if_necessary(const planners::StateSpace& state_space, models::RelationalNeuralNetwork& model)
    {
    ...
    torch::Tensor
    PolicyIterativeTabularEvaluation::loss(const planners::StateSpaceSampleList& batch, const PolicyBatchOutput& output, models::RelationalNeuralNetwork& model)
    {
        ...
        for (uint32_t sample_index = 0; sample_index < batch.size(); ++sample_index)
        {
            ...
            const auto sampled_successor_value = value_vector[samled_successor_index];
            const auto state_baseline = disable_baseline_ ? 0.0 : (value_vector[state_index] - 1.0);
            // TODO: Use every successor state? This seems unncessary.
            total_policy_loss += (sampled_successor_value - state_baseline) * sampled_successor_probability.log();
        }
    ...
    std::shared_ptr<torch::optim::Optimizer> PolicyIterativeTabularEvaluation::create_optimizer(models::RelationalNeuralNetwork& model)
    {
        return std::make_shared<torch::optim::Adam>(model.parameters(), learning_rate_);
    }

    torch::Tensor PolicyIterativeTabularEvaluation::train_loss(uint32_t epoch,
    ...
    PolicyIterativeTabularEvaluation::get_batch(const std::shared_ptr<datasets::Dataset>& set, uint32_t epoch, uint32_t batch_index, uint32_t batch_size)
    {
